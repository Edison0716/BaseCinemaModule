 ...
 * @author: 巴黎没有摩天轮Li
 * @description: 状态栏工具类
 * @date: Created in 上午9:45 2017/11/14
 * @modified by:ids.xml
 */
...
object StatusBarUtil {
     ...
     * @param drawerLayout DrawerLayout
     */
    private fun setTransparentForDrawerLayout(activity: Activity, drawerLayout: DrawerLayout) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            return
        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
     ...
     * @param needOffsetView 需要向下偏移的 View
     */
    private fun setTranslucentForImageView(activity: Activity, @IntRange(from = 0, to = 255) statusBarAlpha: Int,
                                           needOffsetView: View?) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            return
        }
